...
class FDPOMixedDataModule(pl.LightningDataModule):
    def __init__(
        self,
        env_name: str,
        agent: str,
    ...
    def path_to_data(self) -> str:
        mix = str(round(float(self.mix), 2)).replace(".", "_")
        return str(
            relative_to_file(
                __file__,
                f"../../data/dataset/FDPOMixture/{self.env_name}/mix_{mix}/{self.dataset_size}/{self.seed}/dataset.pt",
            ).resolve()
        )
    ...
    def _create_buffer(self):
        root = relative_to_file(
            __file__, f"../../data/agents/online/{self.agent}/{self.env_name}"
        ).resolve()
        checkpoint_path = list(root.rglob("last.ckpt"))[0]

        if "dqn" in self.agent.lower():
            model = DQN.load_from_checkpoint(
                str(checkpoint_path), hparams_file=str(root / "hparams.yaml")
            )
        ...
        def policy(x):
            if len(buffer) < int(self.dataset_size * self.mix):
    ...
    def setup(self, stage: Optional[str] = None) -> Any:
        ...
        if self.bootstrap_prop > 0.0:
            ...
            mask = create_bootstrapping_mask(
                len(self.buffer), self.n_boostrap_heads, self.bootstrap_prop
            )
